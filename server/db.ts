...
export async function saveGeneratedContent(userId: number, input: { campaignId?: number | null; type: string; platform: string; tone: string; title: string; content: string }) { const connection = await database(); const result = await connection.insert(generatedContents).values({ userId, campaignId: input.campaignId ?? null, type: input.type, platform: input.platform, tone: input.tone, title: input.title, content: input.content }); const saved = await connection.select().from(generatedContents).where(and(eq(generatedContents.userId, userId), eq(generatedContents.id, Number(result[0].insertId)))).limit(1); return saved[0]!; }
