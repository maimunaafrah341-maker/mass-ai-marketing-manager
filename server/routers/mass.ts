...
export const massRouter = router({
  ...
  content: router({
    ...
    save: protectedProcedure.input(z.object({ campaignId: z.number().int().positive().optional().nullable(), type: z.string().trim().min(2).max(120), platform: z.string().trim().min(2).max(120), tone: z.string().trim().min(2).max(120), title: z.string().trim().min(1).max(400), content: z.string().trim().min(1).max(10000) })).mutation(async ({ ctx, input }) => {
  ...
  ai: router({
    ...
    generateContent: protectedProcedure.input(contentInput).mutation(async ({ ctx, input }) => {
      ...
      const response = await runAI(
        "You are MASS AI's content studio. Generate accurate, edit-ready marketing copy from supplied business and campaign context. Do not make unsupported claims. Do not say content is published or scheduled. Return only required JSON.",
